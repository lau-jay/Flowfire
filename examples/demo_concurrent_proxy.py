...
from flowfire.defines import L2_BOOK, OPEN_INTEREST, BINANCE, BINANCE_FUTURES
from flowfire.exchanges import Binance, BinanceFutures
...
def main(proxy):
    ...
    f.add_feed(
        Binance(
            ...
            channels=[L2_BOOK],
            callbacks={
                L2_BOOK: counter.callback(BINANCE, L2_BOOK, book_symbols, False)
            },
        )
    )
    f.add_feed(
    ...
    f.add_feed(
        BinanceFutures(
            ...
            callbacks={
                OPEN_INTEREST: counter.callback(
                    BINANCE_FUTURES, OPEN_INTEREST, futures_symbols, False
                )
            },
        )
    )
    f.add_feed(
        BinanceFutures(
            ...
            callbacks={
                OPEN_INTEREST: counter.callback(
                    BINANCE_FUTURES, OPEN_INTEREST, futures_symbols, True
                )
            },
        )
    )
