...
from flowfire.backends.redis import (
    BookRedis,
    BookStream,
    CandlesRedis,
    FundingRedis,
    OpenInterestRedis,
    TradeRedis,
    BookSnapshotRedisKey,
)
from flowfire.defines import CANDLES, FUNDING, L2_BOOK, OPEN_INTEREST, TRADES
from flowfire.exchanges import Bitfinex, Bitmex, Coinbase, Gemini, Binance
...
def main():
    ...
    f.add_feed(
        Bitfinex(
            channels=[TRADES], symbols=["BTC-USD"], callbacks={TRADES: TradeRedis()}
        )
    )
    f.add_feed(
        Coinbase(
    ...
    f.add_feed(
        Coinbase(
            channels=[L2_BOOK], symbols=["BTC-USD"], callbacks={L2_BOOK: BookStream()}
        )
    )
