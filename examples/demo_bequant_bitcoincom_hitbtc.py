...
from flowfire.connection import AsyncConnection
from flowfire.callback import BalancesCallback, TransactionsCallback, TickerCallback
import pprint
...
def main():
    ...
    f.add_feed(
        BEQUANT, channels=[L2_BOOK], symbols=["ALGO-USDT"], callbacks={L2_BOOK: (book)}
    )
    f.add_feed(
        HITBTC, channels=[L2_BOOK], symbols=["ATOM-USDT"], callbacks={L2_BOOK: (book)}
    )
    f.add_feed(
        BEQUANT,
        channels=[CANDLES],
    ...
    f.add_feed(
        BEQUANT, channels=[TRADES], symbols=["XLM-USDT"], callbacks={TRADES: trade}
    )
    f.add_feed(
        HITBTC, channels=[TRADES], symbols=["DASH-USDT"], callbacks={TRADES: trade}
    )
