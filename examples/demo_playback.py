...
from flowfire.defines import COINBASE, L2_BOOK, TRADES, TICKER, BID, ASK
from flowfire.raw_data_collection import playback
...
async def trade(trade, receipt_timestamp):
    print(
        f"Timestamp: {trade.timestamp} flowfire Receipt: {receipt_timestamp} Exchange: {trade.exchange} Symbol: {trade.symbol} ID: {trade.id} Side: {trade.side} Amount: {trade.amount} Price: {trade.price}"
    )
...
def main():
    ...
    print(pcaps)
    stats = playback(
        COINBASE, pcaps, callbacks={L2_BOOK: book, TICKER: ticker, TRADES: trade}
    )
