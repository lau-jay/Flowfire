...
from flowfire.defines import BID, ASK, COINBASE, L2_BOOK, TICKER, TRADES
from flowfire.exchanges import Binance, Coinbase
...
async def book(update, receipt_timestamp):
    ...
    if update.delta:
        print(
            f"Delta from last book contains {len(update.delta[BID]) + len(update.delta[ASK])} entries."
        )
    else:
...
def main():
    ...
    f.add_feed(
        COINBASE, symbols=["BTC-USD"], channels=[TICKER], callbacks={TICKER: ticker}
    )
    f.add_feed(
        Coinbase(symbols=["BTC-USD"], channels=[TRADES], callbacks={TRADES: trade})
    )
    f.add_feed(
        Coinbase(
            subscription={
