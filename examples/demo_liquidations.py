...
from flowfire.defines import LIQUIDATIONS
from flowfire.exchanges import EXCHANGE_MAP


async def liquidations(data, receipt):
    print(
        f"flowfire Receipt: {receipt} Exchange: {data.exchange} Symbol: {data.symbol} Side: {data.side} Quantity: {data.quantity} Price: {data.price} ID: {data.id} Status: {data.status}"
    )
...
def main():
    ...
    for exchange_string, exchange_class in EXCHANGE_MAP.items():
        if LIQUIDATIONS in exchange_class.info()["channels"]["websocket"]:
            ...
            f.add_feed(
                exchange_class(
                    subscription={LIQUIDATIONS: symbols},
