...
from flowfire.exchanges.bitdotcom import BitDotCom
from flowfire.exchanges.bitget import Bitget
from flowfire.exchanges.cryptodotcom import CryptoDotCom
from flowfire.exchanges.delta import Delta
from flowfire.exchanges.fmfw import FMFW
from flowfire.exchanges.independent_reserve import IndependentReserve
from flowfire.exchanges.kraken_futures import KrakenFutures
from flowfire.exchanges.blockchain import Blockchain
from flowfire.exchanges.bithumb import Bithumb
from flowfire.symbols import Symbol
from flowfire.exchanges.phemex import Phemex
from flowfire.exchanges.dydx import dYdX
from flowfire.exchanges.deribit import Deribit
...
async def book(book, receipt_timestamp):
    ...
    if book.delta:
        print(
            f"Delta from last book contains {len(book.delta[BID]) + len(book.delta[ASK])} entries."
        )
...
def main():
    ...
    f.add_feed(
        Bitfinex(
            symbols=["BTC-USDT"],
            channels=[L3_BOOK],
            callbacks={L3_BOOK: book, TICKER: ticker, TRADES: trade},
        )
    )
    f.add_feed(
        Bitflyer(
            ...
            callbacks={L2_BOOK: book, TICKER: ticker, TRADES: trade},
        )
    )
    f.add_feed(
        Bithumb(symbols=["BTC-KRW"], channels=[TRADES], callbacks={TRADES: trade})
    )
    f.add_feed(
        Bitmex(
            timeout=5000,
            ...
            callbacks={LIQUIDATIONS: liquidations, OPEN_INTEREST: oi, FUNDING: funding},
        )
    )
    f.add_feed(
        Bitstamp(
    ...
    f.add_feed(
        BLOCKCHAIN,
        subscription={L2_BOOK: ["BTC-USD"], TRADES: Blockchain.symbols()},
        callbacks={L2_BOOK: book, TRADES: trade},
    )
    f.add_feed(
        Bybit(
            ...
            callbacks={OPEN_INTEREST: oi, INDEX: index, FUNDING: funding},
        )
    )
    f.add_feed(
        Bybit(
            ...
            callbacks={CANDLES: candle_callback, TRADES: trade, L2_BOOK: book},
        )
    )
    f.add_feed(
        Coinbase(
            ...
            callbacks={TRADES: trade, L2_BOOK: book, TICKER: ticker},
        )
    )
    ...
    f.add_feed(
        HitBTC(channels=[TRADES], symbols=["BTC-USDT"], callbacks={TRADES: trade})
    )
    f.add_feed(
        Huobi(
            symbols=["BTC-USDT"],
            channels=[CANDLES, TRADES, L2_BOOK],
            callbacks={TRADES: trade, L2_BOOK: book, CANDLES: candle_callback},
        )
    )
    f.add_feed(
        HuobiDM(
    ...
    f.add_feed(
        HuobiSwap(
            ...
            callbacks={FUNDING: funding, TRADES: trade, L2_BOOK: book},
        )
    )
    f.add_feed(
        KrakenFutures(
    ...
    f.add_feed(
        KuCoin(
            ...
            callbacks={CANDLES: candle_callback, TICKER: ticker, TRADES: trade},
        )
    )
    f.add_feed(
        OKX(
            ...
            channels=[TRADES, TICKER, FUNDING, OPEN_INTEREST, LIQUIDATIONS, L2_BOOK],
            callbacks={
                L2_BOOK: book,
                TICKER: ticker,
                LIQUIDATIONS: liquidations,
    ...
    f.add_feed(
        OKCoin(
            checksum_validation=True,
            symbols=["BTC-USD"],
            channels=[TRADES, TICKER, L2_BOOK, CANDLES],
            callbacks={
                L2_BOOK: book,
                TICKER: ticker,
                TRADES: trade,
    ...
    f.add_feed(
        Poloniex(symbols=["BTC-USDT"], channels=[TRADES], callbacks={TRADES: trade})
    )
    f.add_feed(
        Poloniex(
            subscription={TRADES: ["DOGE-BTC"], L2_BOOK: ["LTC-BTC"]},
            callbacks={TRADES: trade, L2_BOOK: book},
        )
    )
    f.add_feed(
        Probit(
            subscription={TRADES: ["BTC-USDT"], L2_BOOK: ["BTC-USDT"]},
            callbacks={TRADES: trade, L2_BOOK: book},
        )
    )
    f.add_feed(
    ...
    f.add_feed(
        Delta(
            ...
            callbacks={TRADES: trade, CANDLES: candle_callback, L2_BOOK: book},
        )
    )
    f.add_feed(
        BitDotCom(
    ...
    f.add_feed(
        Bitget(
            checksum_validation=True,
            config="config.yaml",
            symbols=["BTC-USDT", "BTC-USDT-PERP", "BTC-USD-PERP"],
    ...
    f.add_feed(
        IndependentReserve(
            ...
            callbacks={TRADES: trade, L3_BOOK: book},
        )
    )
