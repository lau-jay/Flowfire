...
from flowfire.defines import GOOD_TIL_CANCELED, L2_BOOK, LIMIT, SELL, TICKER, TRADES
from flowfire.exchanges import Binance, BinanceDelivery, BinanceFutures
...
async def abook(book, receipt_timestamp):
    print(
        f"BOOK lag: {receipt_timestamp - book.timestamp} Timestamp: {datetime.fromtimestamp(book.timestamp)} Receipt Timestamp: {datetime.fromtimestamp(receipt_timestamp)}"
    )
...
def main():
    ...
    order = binance.place_order_sync(
        "BTC-USDT",
        SELL,
        LIMIT,
        0.002,
    ...
    order = binance_futures.place_order_sync(
        "ETH-USDT-PERP", SELL, LIMIT, 20, 5000, time_in_force=GOOD_TIL_CANCELED
    )
    ...
    order = binance_delivery.place_order_sync(
        "ETH-USD-PERP",
        SELL,
        LIMIT,
        0.05,
