...
from flowfire.exceptions import MissingSequenceNumber
from flowfire.feed import Feed
from flowfire.symbols import Symbol
from flowfire.exchanges.mixins.bitfinex_rest import BitfinexRestMixin
from flowfire.types import Ticker, Trade, OrderBook
...
class Bitfinex(Feed, BitfinexRestMixin):
    ...
    websocket_endpoints = [
        WebsocketEndpoint("wss://api-pub.bitfinex.com/ws/2", limit=20)
    ]
    rest_endpoints = [
        RestEndpoint(
            ...
            for chan in set(channels or subscription):
                for pair in set(subscription[chan] if subscription else symbols or []):
                    ...
                    if (exch_sym[0] == "f") == (chan != FUNDING):
                        LOG.warning(
                            "%s: No %s for symbol %s => flowfire will subscribe to the wrong channel",
                            self.id,
                            chan,
                            pair,
                        )
