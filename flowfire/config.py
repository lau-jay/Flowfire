...
class Config:
    def __init__(self, config=None):
        ...
        elif os.environ.get("FLOWFIRE_CONFIG") and os.path.exists(
            os.environ.get("FLOWFIRE_CONFIG")
        ):
            config = os.environ.get("FLOWFIRE_CONFIG")
            with open(config) as fp:
                self.config = AttrDict(yaml.safe_load(fp))
                self.log_msg = f"Config: use file={config!r} from FLOWFIRE_CONFIG containing the following main keys: {', '.join(self.config.keys())}"
        else:
