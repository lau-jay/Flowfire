...
from flowfire.backends.backend import BackendBookCallback, BackendCallback, BackendQueue
...
class PostgresCallback(BackendQueue):
    def __init__(
        ...
        **kwargs,
        ...
        custom_columns: dict
            A dictionary which maps flowfire's data type fields to Postgres's table column names, e.g. {'symbol': 'instrument', 'price': 'price', 'amount': 'size'}
            Can be a subset of flowfire's available fields (see the cdefs listed under each data type in types.pyx). Can be listed any order.
