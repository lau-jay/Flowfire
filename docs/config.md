...
* An env var
  - An environment variable can be specified to point to the configuration. if `FLOWFIRE_CONFIG` is set, the value in this env var is used as a path to a config file. It is assumed the path is an absolute path and that it also contains the file name.
