...
from flowfire.defines import ASK, BID, KRAKEN
from flowfire.exchanges.kraken import Kraken
...
class TestKrakenRest:
    ...
    def test_historical_trades(self):
        trades = []
        for t in kraken.trades_sync(
            "BTC-USD", start="2021-01-01 00:00:01", end="2021-01-01 00:00:05"
        ):
            trades.extend(t)
        ...
        trades = []
        for t in kraken.trades_sync(
            "BTC-USD", start="2021-01-01 00:00:01", end="2021-01-01 01:00:00"
        ):
            trades.extend(t)
        assert len(trades) == 2074

    @pytest.mark.skipif(
        not kraken.key_id or not kraken.key_secret, reason="No api key provided"
    )
