...
from flowfire.defines import BUY, CANCELLED, LIMIT, PENDING
from flowfire.exchanges import Coinbase
from flowfire.types import Candle, Ticker, Trade
...
class TestCoinbaseRest:
    ...
    def test_trade_history_specific_time(self):
        ...
        ret = []
        for data in public.trades_sync(
            "BTC-USD", start="2019-02-13 12:59:10", end="2019-02-13 12:59:17"
        ):
            ret.extend(data)
    ...
    @pytest.mark.skipif(
        sandbox.key_id is None or sandbox.key_secret is None,
    ...
    def test_order_status(self):
        order_resp = sandbox.place_order_sync(
            symbol="BTC-USD", side=BUY, order_type=LIMIT, amount="1.0", price="1.13"
        )
